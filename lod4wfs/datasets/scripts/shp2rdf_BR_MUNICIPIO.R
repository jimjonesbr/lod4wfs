...
basePrefix <- "my"
subject <- paste(basePrefix,":GEOMETRY_MUNICIPALITY_",sep="")
featureSubject <- paste(basePrefix,":FEATURE_MUNICIPALITY_",sep="")

geometryPredicate <- "geo:asWKT " 

shpFolder <- "/home/jones/Desktop/BR_MUNICIPIO/" 
shpName <- "BR_MUNICIPIO" # Shapefile name, WITHOUT extension.


geometryID <- 0 # Column number of the geometry ID in the DBF file, or 0 for using records sequencial.
outputFile <- "/home/jones/Desktop/brazil_municipalities.nt"

#Named Graph information.
namedGraph <- "<http://ifgi.lod4wfs.de/layer/brazil_municipalities>"
abstract <- "Polygons representing all municipalities of the Brazilian Federal States."
title <- "Brazilian Municipalities"
keywords <- "brazil, linked open data, lod4wfs"
...
mapping[3] <- "my:codigo"
mapping[4] <- "my:area_1"
mapping[5] <- "my:perimetro"
mapping[6] <- "my:geocodigo"
mapping[7] <- "my:nome"
mapping[8] <- "my:sede"
mapping[9] <- "my:latitudese"
mapping[10] <- "my:longitudese"
mapping[11] <- "my:area_total"
mapping[12] <- "my:shape_area"
mapping[13] <- "my:shape_len"
